...
func NewBotStorage(pool *pgxpool.Pool, config *config.Config, logger *zap.SugaredLogger) *BotStorage {
	logger = logger.Named("storage")
	return &BotStorage{pool: pool, config: config, logger: logger}
}
...
func (b *BotStorage) Save(ctx context.Context, message *models.Message) error {
	...
	if !ok {
		b.logger.Debugln("no deadline in context")
		return nil
	}
	b.logger.Debugf("saved message %d, time left: %v", message.MessageID, time.Until(deadline))
	return nil
}
...
func (b *BotStorage) GetMsgIDs(ctx context.Context, id int64) ([]int, error) {
	...
	if !ok {
		b.logger.Debugln("deadline not set for get messages ids")
		return result, nil
	}

	b.logger.Debugf("get messages ids, from %d, time left: %s", id, time.Until(deadline))
	return result, nil
}
